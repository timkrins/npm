...
function diffTrees (oldTree, newTree) {
  ...
  function addDifference (flatname, action, pkg) {
  ...
  Object.keys(flatNewTree).forEach(function (flatname) {
    ...
    pkg.flatname = flatname
    pkg.oldPkg = flatOldTree[flatname]
  ...
  Object.keys(flatNewTree).forEach(function (flatname) {
    var pkg = flatNewTree[flatname]
    var oldParent = flatOldTree[pkg.parent && pkg.parent.flatname]
    // modules that are being installed _right now_ via a bundle need to be rebuilt
    // rebuilds are like installs, but lack the extract step
    if (pkg.fromBundle && !pkg.oldPkg && !oldParent && pkg.parent && !pkg.parent.fromBundle) {
      if (npm.config.get('rebuild-bundle')) addDifference(flatname, 'rebuild', pkg)
    // modules that previously existed at this location get updated
    } else if (pkg.oldPkg) {
      if (!pkg.directlyRequested && pkgAreEquiv(pkg.oldPkg.package, pkg.package)) return
      if (!pkg.isInLink && (isLink(pkg.oldPkg) || isLink(pkg))) {
        addDifference(flatname, 'update-linked', pkg)
      } else {
        addDifference(flatname, 'update', pkg)
      }
    } else {
      var vername = getNameAndVersion(pkg.package)
      // modules new to this location that previously existed elsewhere in the tree must be the result
      // of a move
      if (toRemoveByNameAndVer[vername] && toRemoveByNameAndVer[vername].length) {
        ...
        addDifference(flatname, 'move', pkg)
        delete toRemove[flatname]
      // failing all that, we add
      } else {
        addDifference(flatname, 'add', pkg)
      }
    }
  })

  // we only convert removes to differences here so that we can have an
  // opportunity to merge them with adds above and create moves.
  Object
    .keys(toRemove)
    .map(function (flatname) { return toRemove[flatname] })
